...
int32_t main()
{
    ...
    while (t--)
    {
        ...
        vector<vector<int>>dp(target+1,vector<int>(n,1000000000));
        dp[0][0] = 1;
        for(int i=0;i<n;i++)
        {
            dp[0][i]=0;
        ...
        int minn=1000000000;
        // debug(dp);
        vector<int>v(target+1,100000000);
        for (int i = 1; i <= target; i++)
        {
            for (int j = 1; j < n; j++)
            {
                if (i - c[j] >= 0)
                {
                    ...
                    dp[i][j] = min(min(dp[i][j],dp[i][j-1]), min(dp[i - c[j]][j]+1 ,dp[i - c[j]][j-1]+1));
        ...
        cout << (dp[target][n-1] == 1e9 ? -1 : dp[target][n-1]) << endl;
